let isCountingDown=false;$(document).ready(function(){$("#forgot-password-email").on("input change",function(){validateEmail();});startCountDown();});function validateEmail(){if(isCountingDown){return;}
const email=$("#forgot-password-email").val().trim();const feedback=$("#forgot-password-feedback");const button=$("#send-forgot-password-email-button");feedback.removeClass("failed warning info success");feedback.text("");if(!email.match(/^[^\s@]+@[^\s@]+\.[^\s@]+$/)){feedback.addClass("failed").text("Please enter a valid email address.");button.prop("disabled",true);return;}
