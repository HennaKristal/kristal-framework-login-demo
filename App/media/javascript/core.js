...
$.getJSON(kristal_translation_url,(data)=>{kristal_translations=data;kristal_initTranslations();}).fail(()=>{console.error("Failed to find translations file!\n\nTried to look at url:\n"+kristal_translation_url+"\n\nAlternatively, you may have an error in your JSON format.");});});function kristal_initTranslations(){kristal_updateTranslations();$("[switchLanguage]").click(function(event){event.preventDefault();$("#"+kristal_language+"-button").removeClass("active");kristal_language=$(event.currentTarget).attr("switchLanguage");localStorage.setItem(KRISTAL_LANGUAGE_KEY,kristal_language);kristal_updateTranslations();});}
