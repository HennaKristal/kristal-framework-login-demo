$(document).ready(function () {
    ...
    updateActivationLink($("#reset-email").val().trim());

    $("#reset-email").on("input", function () {
        updateActivationLink($(this).val().trim());
    });
});


function updateActivationLink($email) {
    $("#activation-link").attr("href", "/activate-account/" + $email);
}


function startCountDown() {
    const feedback = $("#activate-accound-feedback");
